...
function test1(input, restrictions) {
    let sum = 0;
    let currentGame = 0;
    for (const line of input.split("\n")) {
        ++currentGame;
        const [gameNumberPart, games] = line.split(":");
        let isValid;
        for (const game of games.split(";")) {
            ...
            for (const draw of draws) {
                ...
                if (parseInt(number, 10) > restrictions[color]) {
                    isValid = false;
                    break;
                }
            }
    ...
    for (const line of input.split("\n")) {
        ...
        for (const game of games.split(";")) {
            ...
            for (const draw of draws) {
                ...
                max[color] = Math.max(parsedNumber, max[color]);
            }
        }
...
console.log(`test1 result: ${test1(input, {red: 12, green: 13, blue: 14})}`);
console.log(`test2 result: ${test2(input)}`);